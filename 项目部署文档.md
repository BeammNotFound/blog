...
```shell
SOURCE_PATH=/home/aurora/aurora-service
...
```sh
docker run  --name aurora-springboot-0.0.1.jar -d -p 8888:8888 aurora-springboot-0.0.1.jar:latest
```
...
2. 在/usr/local/nginx下创建nginx.conf文件，格式如下
   ...
   server {
           ...
           location / {		
               root   /home/aurora/aurora-blog;
               index  index.html index.htm; 
               try_files $uri $uri/ /index.html;	
           }
   			
   	location ^~ /api/ {		
               proxy_pass http://43.138.17.194:8888/;
...
2. 修改配置文件
   ...
   server {
           ...
           listen  443 ssl;
           server_name  www.beamstark.cn;
   
           ssl on;
           ssl_certificate /etc/nginx/cert/beamstark.cn_bundle.pem; 
   	   ssl_certificate_key /etc/nginx/cert/beamstark.cn.key; 
   ...
   server {
           ...
           listen  443 ssl;
           server_name admin.beamstark.cn;
   
           ssl on;
           ssl_certificate /etc/nginx/cert/admin.beamstark.cn_bundle.pem; 
           ssl_certificate_key /etc/nginx/cert/admin.beamstark.cn.key; 
           ssl_session_timeout 5m;
           ssl_ciphers ECDHE-RSA-AES128-GCM-SHA256:ECDHE:ECDH:AES:HIGH:!NULL:!aNULL:!MD5:!ADH:!RC4;
           ssl_protocols TLSv1 TLSv1.1 TLSv1.2;
           ssl_prefer_server_ciphers on;
